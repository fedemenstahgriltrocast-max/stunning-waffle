...
4. **Secure Communications** – Implement signed request envelopes (now scaffolded
   via `shared/protocol.ts`) or mTLS between Workers. Use Cloudflare Queues for
   asynchronous tasks (e.g., Thorium scans) to avoid synchronous coupling.
...
- **Internal APIs** – When layers must communicate, prefer `service bindings`
  instead of raw HTTP calls to stay within Cloudflare's secure mesh. Sign each
